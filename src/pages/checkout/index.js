...
import { delete_cart } from "../../redux/slice/cartSlice";
...
const Checkout = () => {
  ...
  //     const waPhoneNumber = '62895400375100';
    ...
    const formattedItems = dataCart
      .map((item, index) => {
        return `${index + 1}. ${item.productName} | ${
          item.quantity
        } pcs | Rp. ${formatRibuan(item.price * item.quantity)}`;
      })
      .join("\n    ");
...
`;
    ...
    // 🧹 Kosongkan cart (Redux + localStorage)
    dispatch(delete_cart());

    // 📦 Reset cart context jika masih pakai
    if (context?.setCartItems) context.setCartItems([]);

    // 🚀 Navigate ke halaman cart
    navigate("/cart");
  };

  const handlePlaceOrderConfirmation = () => {
    if (
      formFields.name === "" ||
      formFields.address === "" ||
      formFields.phoneNumber === ""
    ...
    Swal.fire({
      title: "Apakah data checkout sudah sesuai?",
      text: "Pesanan akan dikirim ke WhatsApp!",
      icon: "question",
      ...
      if (result.isConfirmed) {
        placeOrder(); // jalankan fungsi asli
      }
    });
  };
